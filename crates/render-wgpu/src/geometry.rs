...
pub(crate) fn quad_verts(
    ...
    let [r, g, b, a] = color;
    [
        x0, y_top, r, g, b, a, x1, y_top, r, g, b, a, x1, y_bottom, r, g, b, a, x0, y_top, r, g, b,
        a, x1, y_bottom, r, g, b, a, x0, y_bottom, r, g, b, a,
    ]
}
...
pub(crate) fn build_panel_vertices(
    ...
    let tab_bar_frac = if size.height > 0 {
        tab_bar_h / size.height as f32
    } else {
        0.0
    };
    ...
    let sep_color = if snapshot.editor_focused {
    ...
    if tab_bar_h > 0.0 {
        ...
        let tab_active = mix_c(
            add_c(theme.terminal_bg, 0.05),
            theme.separator_focused,
            0.22,
        );
        let add_btn_bg = {
            let [r, g, b, _] = theme.terminal_bg;
            [
                (r + 0.05).clamp(0.0, 1.0),
                (g + 0.10).clamp(0.0, 1.0),
                (b + 0.03).clamp(0.0, 1.0),
                0.90_f32,
            ]
        };
    ...
    if let Some((raw_sr, raw_sc, raw_er, raw_ec)) = snapshot.selection
        ...
        && cell_h_px > 0.0
    {
        let (sel_sr, sel_sc, sel_er, sel_ec) = if (raw_sr, raw_sc) <= (raw_er, raw_ec) {
    ...
    if size.width > 0 && size.height > 0 && cell_h_px > 0.0 {
        ...
        let visible_editor_rows = ((edit_pane_h_px - snapshot.padding_v as f32) / cell_h_px)
            .floor()
            .max(1.0) as usize;
        if snapshot.editor_line_count > visible_editor_rows {
            ...
            let thumb_fraction =
                (visible_editor_rows as f32 / snapshot.editor_line_count as f32).clamp(0.05, 1.0);
            let thumb_h_ndc = thumb_fraction * track_h_ndc;
            let scrollable_range = snapshot
                .editor_line_count
                .saturating_sub(visible_editor_rows) as f32;
    ...
    if let Some((sel_start, sel_end)) = snapshot.editor_selection
        ...
        && cell_h_px > 0.0
    {
        let (s_start, s_end) = if sel_start <= sel_end {
            (sel_start, sel_end)
        } else {
            (sel_end, sel_start)
        };
...
pub(crate) fn build_suggestion_dropdown_bg_verts(
    ...
    if dd.items.is_empty() || size.width == 0 || size.height == 0 {
        return Vec::new();
    }
    ...
    let max_chars = dd.items[dd.scroll_offset..visible_end]
        .iter()
        .map(|s| s.chars().count())
        .max()
        .unwrap_or(10);
