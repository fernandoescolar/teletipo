...
fn classify_word(word: &str, is_first: bool) -> Option<[f32; 3]> {
    if word.starts_with('-') {
        Some(FLAG)
    } else if SH_KEYWORDS.contains(&word) {
        Some(KEYWORD)
    } else if is_first {
        Some(COMMAND)
    } else {
...
fn parse_variable(chars: &[char], start: usize, n: usize, out: &mut [Option<[f32; 3]>]) -> usize {
...
pub fn highlight_shell(text: &str) -> Vec<Option<[f32; 3]>> {
    ...
    while i < n {
        ...
        match ch {
            ...
            '"' | '\'' => {
                i = parse_quoted_string(&chars, i, n, ch, &mut out);
                first_word = false;
            }
            '$' => {
                i = parse_variable(&chars, i, n, &mut out);
            }
            ';' => {
                i += 1;
                if i < n && chars[i] == ';' {
                ...
                || matches!(ch, '_' | '-' | '.' | '/' | '~' | '@' | ':' | '=') =>
            {
                ...
                let color = classify_word(&word, first_word);
