...
Save frequently-used commands as snippets and access them from the command palette (`Cmd+Shift+P`). Snippets support **dynamic placeholders** that generate options from shell commands, which you can select interactively.

Define snippets and placeholder sources in `config.toml`:

```toml
# Define placeholder sources: shell commands that generate lists of values
[[placeholder_sources]]
name = "branch"
...
# Define snippets using {{placeholder}} syntax
[[commands]]
label = "Run tests"
...
# Snippet without category (defaults to "custom")
[[commands]]
label = "List files"
command = "ls -la"
```
...
**Features:**
- **No shell aliases required** — snippets are built into Teletipo, work from anywhere
- **Categories** — organize snippets with optional `category` field; appears as `category: label` in the palette (defaults to `custom: label` if omitted)
- **Dynamic placeholders** — use `{{name}}` syntax; when executed, Teletipo runs the corresponding source command to generate a list of options
- **Free-form input** — if a placeholder has no source defined, you can type a value directly
- **Interactive selection** — navigate placeholder options with `↑`/`↓` arrow keys and press `Enter` to confirm each one
- **Multi-step filling** — complete one placeholder, then advance to the next automatically
- **Optional working directory** — `cwd` field changes directory before execution (defaults to current tab's cwd)
- **Hints** — optional `hint` field shows guidance when the snippet is selected
- **Palette integration** — snippets appear in the command palette under `Snippets…` category, searchable by label

When a snippet with placeholders is executed:
1. Teletipo enters a sub-prompt mode showing the first placeholder name
2. Available options appear as a dropdown (generated by running the source command)
3. Navigate with `↑`/`↓` and press `Enter` to select the current option
4. Move to the next placeholder automatically
5. After filling all placeholders, the command executes with all values substituted
